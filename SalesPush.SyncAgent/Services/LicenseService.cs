...
// "License" section on FrmMain's Configuration tab. Mirrors SaleOrd's
// LicenseService, but this is a standalone desktop agent with no database
// and no DI container — the offline-grace snapshot lives in AgentConfig
// (persisted via ConfigService to config.json) instead of a DB AppSettings
// table, and the short-lived result cache is a plain instance field instead
// of IMemoryCache.
public class LicenseService
{
    private const string PRODUCT_CODE = "SALESPUSH";
    ...
    public static string GetLicenseKey(AgentConfig config) => ConfigService.Unprotect(config.LicenseKeyProtected);

    public void SaveLicenseKey(AgentConfig config, string plainKey)
    {
        config.LicenseKeyProtected = ConfigService.Protect(plainKey.Trim());
