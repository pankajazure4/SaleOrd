...
// Talks to the same LicenseActivator portal SaleOrd and SyncMast already use
// — this product is registered there under its own ProductCode ("SALESPUSH"),
// so a key issued for SaleOrd/SyncMast won't validate here and vice versa.
// Throws on network failure so callers can distinguish "portal unreachable"
// (fall back to the offline grace period) from "portal said no" (hard fail).
